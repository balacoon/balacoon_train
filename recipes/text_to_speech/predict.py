...
def main():
    ...
    parser.add_argument(
        "--phoneme-mapping", type=str, required=True, help="Path to phoneme mapping"
    )
    ...
    # Create model
    # We need to pass config.model to load_from_checkpoint because TTSALMModel
    # doesn't save hyperparameters automatically
    model = TTSALMModel.load_from_checkpoint(args.ckpt, config=config.model)
