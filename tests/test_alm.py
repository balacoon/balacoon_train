...
def batch_data(config):
    ...
    acoustic_tokens = torch.randint(0, config.dec_vocab_size, (batch_size, acoustic_seq_len, config.dec_vocabs_num))
...
def test_encoder_bidirectionality(model, batch_data):
    ...
    assert is_bidirectional, "Encoder should be bidirectional (output at pos 0 depends on input at pos 5)"
...
def test_decoder_causality(model, batch_data):
    ...
    assert grad_at_future == 0, f"Decoder should be causal, but gradient at future pos is {grad_at_future}"
...
def test_generate(model, batch_data, config):
    ...
    acoustic_prompt = torch.randint(0, config.dec_vocab_size, (batch_size, prompt_len, config.dec_vocabs_num))
